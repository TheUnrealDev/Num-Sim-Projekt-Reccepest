...
def prop(y, coeff):
    sus, exp, inf, res, ded, vac= y
    alpha, beta, gamma, my, nu, N = coeff
    ...
    return np.array([prob_exp, prob_inf, prob_res, prob_ded, prob_vac])
...
def mc_simulate(values, should_print=False):
    ...
    my = values["my"]  # Död per tidsenhet
    nu = values["nu"] # Vaccinationstakt
    ...
    coeff = [alpha, beta, gamma, my, nu, N] # VARIERAR FRÅN FIL TILL FIL
    x0 = [sus0, exp0, inf0, res0, ded0, vac0]
    ...
    for i in range(num_samples):
        ...
        V = np.interp(t_eval, sol_x, sol_y[:, 5])
